...
export function PiperTTSModule() {
  const [texto, setTexto] = useState("");
  const [voz, setVoz] = useState("pt_BR-cadu-medium");
  const [serverUrl, setServerUrl] = useState("http://192.168.0.42:8000");
  ...
  const gerarAudio = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('piper-tts-proxy', {
        body: {
          texto: texto.trim(),
          voz: voz,
          api_url: serverUrl
      ...
      if (data.success) {
        ...
        setCurrentAudio(data.url);

        toast({
          title: "✅ Áudio gerado!",
          description: `Voz ${data.voz} processada com sucesso`,
        });
      } else {
      ...
      toast({
        ...
        description: error.message || "Verifique se a API Piper está rodando",
        variant: "destructive",
      });
    } finally {
  ...
  const playPauseAudio = (url?: string) => {
    const audioToPlay = url || currentAudio;
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">🎤 Piper TTS - Voz Neural Gratuita</h2>
          <p className="text-muted-foreground">Gere áudios realistas em português sem limites</p>
        </div>
      </div>
                  ...
                  {audio.status === 'success' && (
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => playPauseAudio(audio.url)}
                      >
                        <Play className="h-4 w-4" />
                      </Button>
                      <Button
