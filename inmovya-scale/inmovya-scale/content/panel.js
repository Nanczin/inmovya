...
window.IS.Panel = {
    ...
    this.container.innerHTML = `
      ...
      <div id="is-sidebar" class="${this.isOpen ? 'is-open' : ''}" style="width: ${this.settings.panelWidth}px">
        ...
        <div class="is-filters">
          <div class="is-search-wrapper" style="display:flex;gap:8px;margin-bottom:8px;">
            <input type="text" id="is-search-input" placeholder="Pesquisar respostas..." style="flex:1" />
            <button id="is-add-btn" title="Nova Resposta" style="background:var(--inmovya-primary);color:white;border:none;border-radius:8px;padding:0 12px;cursor:pointer;font-weight:bold;font-size:18px;">+</button>
          </div>
          
          <div id="is-add-form" style="display:none; flex-direction:column; gap:8px; margin-bottom: 8px; padding: 12px; background: var(--inmovya-hover); border: 1px solid var(--inmovya-border); border-radius: 8px;">
            <input type="text" id="is-add-title" placeholder="Título" style="padding:8px; border:1px solid var(--inmovya-border); border-radius:4px; background:var(--inmovya-surface); color:var(--inmovya-text);" />
            <input type="text" id="is-add-shortcut" placeholder="Atalho (ex: /oi)" style="padding:8px; border:1px solid var(--inmovya-border); border-radius:4px; background:var(--inmovya-surface); color:var(--inmovya-text);" />
            <textarea id="is-add-message" placeholder="Mensagem (use {{nome}})..." rows="4" style="padding:8px; border:1px solid var(--inmovya-border); border-radius:4px; background:var(--inmovya-surface); color:var(--inmovya-text); font-family:inherit;"></textarea>
            <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:4px;">
              <button id="is-cancel-add" style="padding:6px 12px; cursor:pointer; background:transparent; border:1px solid var(--inmovya-border); border-radius:4px; color:var(--inmovya-text);">Cancelar</button>
              <button id="is-save-add" style="padding:6px 12px; background:var(--inmovya-primary);color:white;border:none;border-radius:4px;cursor:pointer;">Salvar</button>
            </div>
          </div>
  ...
  setupListeners() {
    ...
    const addBtn = document.getElementById('is-add-btn');
    const cancelAddBtn = document.getElementById('is-cancel-add');
    const saveAddBtn = document.getElementById('is-save-add');
    const addForm = document.getElementById('is-add-form');
    ...
    if (saveAddBtn) {
      saveAddBtn.addEventListener('click', async () => {
        const title = document.getElementById('is-add-title').value.trim();
        let shortcut = document.getElementById('is-add-shortcut').value.trim();
        const message = document.getElementById('is-add-message').value.trim();
        
        if (!title || !message) {
          this.showToast("Título e Mensagem são obrigatórios!");
          return;
        }
        ...
        document.getElementById('is-add-title').value = '';
        document.getElementById('is-add-shortcut').value = '';
        document.getElementById('is-add-message').value = '';
        addForm.style.display = 'none';
        
        this.updateList();
        this.showToast("Resposta criada!");
      });
    }
